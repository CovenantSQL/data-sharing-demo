...
define([], function () {
    ...
    this.g().selectAll(".node").data(nodes, function (d) {
      ...
      .call(function () {
        var transform = function (d) {
          ...
          stroke = {
            dash: function (d) {
              return (d.state() === "candidate" ? "5,5" : "");
            },
            opacity: function (d) {
              return (d.state() === "follower" || d.state() === "stopped" ? 0 : 1);
            },
          };
        ...
        g.each(function (node) {
          ...
          // Log
          var fixedData = node.log().slice(start, logSize);
          d3.select(this).select("g.log").selectAll("g.log-entry").data(fixedData)
            .call(function () {
              var transform = function (d) {
                return "translate(" + self.parent().scales.size(d.dx) + "," + (self.parent().scales.size(d.dy) - self.parent().scales.y(4*start)) + ")"
              };
              var text = {
                x: function (d) {
                ...
                fill: function (d) {
                  return (d.index <= node.commitIndex() ? "black" : "red");
                },
              };
  ...
  NodeLayout.prototype.invalidateElectionTimers = function () {
    var self = this,
      model = this.parent().model(),
      nodes = this.nodes(),
      electionAt = function (d) {
        return d.electionTimer() !== null ? d.electionTimer().startTime() : 0;
      };
